"""
This module provides the Port class which describes an API used for talking with
serial like devices.
"""
...
class Bus:
    ...
    def send_read(self, dev_id, offset, num_bytes):
        ...
        """
        if self.show & Bus.SHOW_COMMANDS:
            log('Sending READ to ID {} offset 0x{:02x} len {}'.format(dev_id, offset, num_bytes))
    ...
    def send_write(self, dev_id, offset, data, deferred=False):
        ...
        if self.show & Bus.SHOW_COMMANDS:
            ...
            if dev_id == packet.Id.BROADCAST:
                ...
                log(
                        'Sending {} to ID {} offset 0x{:02x} len {}'.format(
                                cmd_str,
                                dev_id,
                                offset,
                                len(data)
    ...
    def sync_write(self, dev_ids, offset, values):
        ...
        if self.show & Bus.SHOW_COMMANDS:
            ...
            log(
                    'Sending SYNC_WRITE to IDs {} offset 0x{:02x} len {}'.format(
                            ids,
                            offset,
                            len(values[0])
                    )
        ...
        if num_ids != len(values):
            raise ValueError(
                    'len(dev_ids) = {} must match len(values) = {}'.format(num_ids,
                                                                           len(values))
            )
